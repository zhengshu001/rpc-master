 ...
 */
public class ClientClassPathScanner extends ClassPathBeanDefinitionScanner {

    private DefaultListableBeanFactory beanFactory;
    public ClientClassPathScanner(BeanDefinitionRegistry registry) {
    ...
    public Set<BeanDefinitionHolder> doScan(String... basePackages) {
        ...
        if (beanDefinitions.isEmpty()) {
            logger.warn("No grpc client was found in '" + Arrays.toString(basePackages) + "' package. Please check your configuration.");
        } else {
            for (BeanDefinitionHolder holder : beanDefinitions) {
                GenericBeanDefinition definition = (GenericBeanDefinition) holder.getBeanDefinition();
                String beanClassName = definition.getBeanClassName();
                if (logger.isInfoEnabled()) {
                    logger.info("Creating ClientFactoryBean with name '" + holder.getBeanName() + "' and '" +  beanClassName + "' RpcInterface");
                }
                try {
                    if (beanClassName.endsWith(GrpcUtils.GRPC_SERVICE_IMPL_SUFFIX)) {
                        ...
                        if (logger.isInfoEnabled()) {
                            logger.info("Creating ClientFactoryBean rpcInterface [" + rpcInterface + "] grpcServiceImpl [" +  beanClassName + "] grpcServiceOuter [" + grpcServiceOuter + "]");
                        }
